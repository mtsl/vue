...
public class StaggeredGridView extends AdapterView<ListAdapter> {
    ...
    public StaggeredGridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            mColCount = a.getInteger(
                    R.styleable.StaggeredGridView_sgv_numColumns, 2);
            mDrawSelectorOnTop = a.getBoolean(
                    R.styleable.StaggeredGridView_sgv_drawSelectorOnTop, false);
            mItemMargin = (int) a.getDimension(
                    R.styleable.StaggeredGridView_sgv_itemMargin, 0);
        } else {
            mColCount = 2;
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (child != null && !child.hasFocusable() && inList) {
                    ...
                    if (mTouchMode == TOUCH_MODE_DOWN
                        ...
                        return true;
                    } else if (!mDataChanged && mAdapter != null
                            && mAdapter.isEnabled(motionPosition)) {
                        performClick.run();
                    }
                }
    ...
    private boolean trackMotionScroll(int deltaY, boolean allowOverScroll) {
        ...
        if (mOnScrollListener != null) {
            mOnScrollListener.onScroll(null, getFirstPosition(),
                    getChildCount(), this.mItemCount);
        }
    ...
    private void recycleOffscreenViews() {
        ...
        for (int i = getChildCount() - 1; i >= 0; i--) {
            ...
            // if (column >= 0 && column < mItemBottoms.length &&
            // itemsPerColumn[column] == 1) {
            // break;
            // }
            // }
            if (mInLayout) {
