...
public class CustomViewAbove extends ViewGroup {
  ...
  public void computeScroll() {
    // Log.i("homeclick", "homeclick in lib toggle computeScroll() 12");
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    switch (action) {
      case MotionEvent.ACTION_MOVE:
        ...
        if (DEBUG)
          Log.v(TAG, "onInterceptTouch moved to:(" + x + ", " + y + "), diff:("
              + xDiff + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
    ...
    switch (action & MotionEventCompat.ACTION_MASK) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (!mIsBeingDragged) {
          ...
          final float yDiff = Math.abs(y - mLastMotionY);
          if (DEBUG)
            Log.v(TAG, "onTouch moved to:(" + x + ", " + y + "), diff:("
                + xDiff + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
          if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed(dx)) {
  ...
  public void scrollTo(int x, int y) {
    super.scrollTo(x, y);
    mScrollX = x;
    if (mEnabled) mCustomViewBehind.scrollBehindTo(mContent, x, y);
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    boolean despatchEvent = false;
    try{
    // Let the focused view and/or our descendants get the key first
    despatchEvent = super.dispatchKeyEvent(event) || executeKeyEvent(event);
    } catch(Exception e) {
      e.printStackTrace();
    }
    return despatchEvent;
  }
