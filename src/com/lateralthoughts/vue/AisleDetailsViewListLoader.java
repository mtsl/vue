...
public class AisleDetailsViewListLoader {
    ...
    AisleContentBrowser mContentBrowser = null;
    ...
    AisleDetailsViewListLoader(Context context) {
        ...
        mViewFactory = ScaledImageViewFactory.getInstance(VueApplication
                .getInstance());
        mBitmapLoaderUtils = BitmapLoaderUtils.getInstance();
        mContentAdapterFactory = ContentAdapterFactory
                .getInstance(VueApplication.getInstance());
    }
    
    public void getAisleContentIntoView(
            AisleDetailsViewAdapter.ViewHolder holder, int scrollIndex,
            int position, DetailClickListener detailListener,
            AisleWindowContent windowContent, boolean setPosistion,
            LinearLayout starImageLay) {
        mBestHeight = 0;
        ...
        mContentBrowser = holder.aisleContentBrowser;
        
        mContentBrowser.setHolderName(VueAisleDetailsViewFragment.SCREEN_NAME);
        ...
        if (holder.uniqueContentId.equals(desiredContentId)) {
            ...
            for (int i = 0; i < mContentBrowser.getChildCount(); i++) {
                // ((ScaleImageView)contentBrowser.getChildAt(i)).setContainerObject(null);
                mViewFactory
                        .returnUsedImageView((ScaleImageView) mContentBrowser
                                .getChildAt(i));
            }
        ...
        if (null != imageDetailsArr && imageDetailsArr.size() != 0) {
            ...
            mContentBrowser.addView(imageView);
            ...
            if (bitmap != null) {
                if (bitmap.getHeight() > mBestHeight) {
                    loadBitmap(itemDetails, mContentBrowser, imageView,
                            mBestHeight, scrollIndex);
                } else {
                }
                imageView.setImageBitmap(bitmap);
                if (scrollIndex != 0) {
                    mContentBrowser.setCurrentImage();
                }
            } else {
                loadBitmap(itemDetails, mContentBrowser, imageView,
                        mBestHeight, scrollIndex);
    ...
    public void loadBitmap(AisleImageDetails itemDetails,
        ...
        BitmapWorkerTask task = new BitmapWorkerTask(itemDetails, flipper,
                imageView, bestHeight, scrollIndex);
    ...
    class BitmapWorkerTask extends AsyncTask<String, Void, Bitmap> {
        ...
        public BitmapWorkerTask(AisleImageDetails itemDetails,
                ...
                int bestHeight, int scrollIndex) {
    ...
    public void clearBrowser(ArrayList<AisleImageDetails> imageList) {
        if (mContentBrowser != null) {
            for (int i = 0; i < mContentBrowser.getChildCount(); i++) {
                ...
                try {
                    ImageView image = (ScaleImageView) mContentBrowser
                            .getChildAt(i);
            ...
            mContentAdapterFactory.returnUsedAdapter(mContentBrowser
                    .getCustomAdapter());
            mContentBrowser.setCustomAdapter(null);
            mContentBrowser.removeAllViews();
    ...
    private void setStarIconParams(AisleWindowContent windowContent,
            LinearLayout starImageLay) {
        ...
        int imageHeight = windowContent.getImageList().get(
                VueApplication.getInstance().getmAisleImgCurrentPos()).mDetailsImageHeight;
        int imageWidth = windowContent.getImageList().get(
                VueApplication.getInstance().getmAisleImgCurrentPos()).mDetailsImageWidth;
        int imageRightSpace = browserWidth - imageWidth;
        int imageTopAreaHeight = (browserHeight / 2) - (imageHeight / 2);
        ...
        starIconParams.setMargins(VueApplication.getInstance().getPixel(4)
                + imageRightSpace / 2, VueApplication.getInstance()
                .getPixel(10) + imageTopAreaHeight, 0, 0);
        
        starImageLay.setLayoutParams(starIconParams);
    }
}