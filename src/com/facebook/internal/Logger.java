...
public class Logger {
    ...
    // Note that the mapping of replaced strings is never emptied, so it should
    // be used only for things that
    // are not expected to be too numerous, such as access tokens.
    public synchronized static void registerStringToReplace(String original,
            String replace) {
    ...
    public synchronized static void registerAccessToken(String accessToken) {
        if (Settings
                .isLoggingBehaviorEnabled(LoggingBehavior.INCLUDE_ACCESS_TOKENS) == false) {
    ...
    public static void log(LoggingBehavior behavior, String tag, String format,
            Object... args) {
    ...
    public static void log(LoggingBehavior behavior, int priority, String tag,
            String string) {
        if (Settings.isLoggingBehaviorEnabled(behavior)) {
    ...
    public void setPriority(int value) {
        Validate.oneOf(value, "value", Log.ASSERT, Log.DEBUG, Log.ERROR,
    ...
    // Immediately logs a string, ignoring any accumulated contents, which are
    // left unchanged.
