...
public class TestSession extends Session {
    ...
    TestSession(Activity activity, List<String> permissions,
        ...
        // Validate these as if they were arguments even though they are
        // statics.
     ...
     * 
     * @param activity
     *            the Activity to use for opening the session
     * @param permissions
     *            list of strings containing permissions to request; nil will
     *            result in a common set of permissions (email, publish_actions)
     *            being requested
     ...
     *            result in a common set of permissions (email, publish_actions)
     *            being requested
     * @param sessionUniqueUserTag
     *            a string which will be used to make this user unique among
     *            other users with the same permissions. Useful for tests which
     *            require two or more users to interact with each other, and
     *            which therefore must have sessions associated with different
     *            users.
     * @return a new TestSession that is in the CREATED state, ready to be
     *         opened
     */
    public static TestSession createSessionWithSharedUser(Activity activity,
            List<String> permissions, String sessionUniqueUserTag) {
        return createTestSession(activity, permissions, Mode.SHARED,
    ...
    public static synchronized void setTestApplicationId(String applicationId) {
        if (testApplicationId != null
                && !testApplicationId.equals(applicationId)) {
            throw new FacebookException(
                    "Can't have more than one test application ID");
        }
     ...
     */
    public static synchronized void setTestApplicationSecret(
            String applicationSecret) {
        if (testApplicationSecret != null
                && !testApplicationSecret.equals(applicationSecret)) {
            throw new FacebookException(
                    "Can't have more than one test application secret");
        }
    ...
    private static synchronized TestSession createTestSession(
            Activity activity, List<String> permissions, Mode mode,
        ...
        return new TestSession(activity, permissions,
                new TestTokenCachingStrategy(), sessionUniqueUserTag, mode);
    }
    ...
    private static synchronized void retrieveTestAccountsForAppIfNeeded() {
        ...
        // The data we need is split across two different FQL tables. We
        // construct two queries, submit them
        // together (the second one refers to the first one), then
        // cross-reference the results.
        
        // Get the test accounts for this app.
        String testAccountQuery = String.format(
                "SELECT id,access_token FROM test_account WHERE app_id = %s",
                testApplicationId);
        ...
        // Build a JSON string that contains our queries and pass it as the 'q'
        ...
        if (fqlResults == null || fqlResults.size() != 2) {
            throw new FacebookException(
                    "Unexpected number of results from FQL query");
        }
        
        // We get back two sets of results. The first is from the test_accounts
        // query, the second from the users query.
        Collection<TestAccount> testAccounts = fqlResults.get(0)
                .getFqlResultSet().castToListOf(TestAccount.class);
        Collection<UserAccount> userAccounts = fqlResults.get(1)
                .getFqlResultSet().castToListOf(UserAccount.class);
    ...
    private static synchronized void populateTestAccounts(
        ...
        // We get different sets of data from each of these queries. We want to
        // combine them into a single data
        // structure. We have added a Name property to the TestAccount
        // interface, even though we don't really get
        // a name back from the service from that query. We stick the Name from
        // the corresponding UserAccount in it.
    ...
    private static synchronized TestAccount findTestAccountMatchingIdentifier(
            String identifier) {
    ...
    public final String toString() {
        ...
        return new StringBuilder().append("{TestSession")
                .append(" testUserId:").append(testAccountId).append(" ")
                .append(superString).append("}").toString();
    }
    ...
    void postStateChange(final SessionState oldState,
            final SessionState newState, final Exception error) {
    ...
    void forceExtendAccessToken(boolean forceExtendAccessToken) {
        ...
        setTokenInfo(new AccessToken(currentToken.getToken(), new Date(),
                currentToken.getPermissions(), AccessTokenSource.TEST_USER,
                new Date(0)));
    ...
    private void finishAuthWithTestAccount(TestAccount testAccount) {
        ...
        AccessToken accessToken = AccessToken.createFromString(
                testAccount.getAccessToken(), requestedPermissions,
    ...
    private TestAccount createTestAccountAndFinishAuth() {
        ...
        // If we're in shared mode, we want to rename this user to encode its
        // permissions, so we can find it later
        // in another shared session. If we're in private mode, don't bother
        // renaming it since we're just going to
        ...
        String graphPath = String.format("%s/accounts/test-users",
                testApplicationId);
        Request createUserRequest = new Request(null, graphPath, parameters,
                HttpMethod.POST);
        ...
        if (error != null) {
            ...
            // If we are in shared mode, store this new account in the
            // dictionary so we can re-use it later.
            if (mode == Mode.SHARED) {
                // Remember the new name we gave it, since we didn't get it back
                // in the results of the create request.
    ...
    private void deleteTestAccount(String testAccountId, String appAccessToken) {
        ...
        Request request = new Request(null, testAccountId, parameters,
                HttpMethod.DELETE);
        Response response = request.executeAndWait();
    ...
    private String getSharedTestAccountIdentifier() {
        ...
        long sessionTagHash = (sessionUniqueUserTag != null) ? sessionUniqueUserTag
                .hashCode() & 0xffffffffL : 0;
    ...
    private String validNameStringFromInteger(long i) {
        ...
        // We know each character is a digit. Convert it into a letter 'a'-'j'.
        // Avoid repeated characters
        // that might make Facebook reject the name by converting every other
        // repeated character into one
        // 10 higher ('k'-'t').
        char lastChar = 0;
    ...
    private interface TestAccount extends GraphObject {
        ...
        // Note: We don't actually get Name from our FQL query. We fill it in by
        // correlating with UserAccounts.
        String getName();
    ...
    private static final class TestTokenCachingStrategy extends
