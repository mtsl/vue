 ...
 * </p>
 * <p>
 * The Session constructor optionally takes a TokenCachingStrategy, from which
 * it will attempt to load a cached token during construction. Also, whenever
 * the Session updates its token, it will also save the token and associated
 * state to the TokenCachingStrategy.
 * </p>
 * <p>
 * This is the only mechanism supported for an Android service to use Session.
 * The service can create a custom TokenCachingStrategy that returns the Session
 * provided by an Activity through which the user logged in to Facebook.
 * </p>
 */
public abstract class TokenCachingStrategy {
     ...
     *             if the passed in Bundle is null
     */
     ...
     * 
     * @throws NullPointerException
     *             if the passed in Bundle or token value are null
     */
     ...
     * 
     * @throws NullPointerException
     *             if the passed in Bundle or date value are null
     */
     ...
     * 
     * @throws NullPointerException
     *             if the passed in Bundle or permissions list are null
     */
    ...
    public static AccessTokenSource getSource(Bundle bundle) {
        ...
        if (bundle.containsKey(TokenCachingStrategy.TOKEN_SOURCE_KEY)) {
            return (AccessTokenSource) bundle
                    .getSerializable(TokenCachingStrategy.TOKEN_SOURCE_KEY);
        } else {
            boolean isSSO = bundle.getBoolean(TokenCachingStrategy.IS_SSO_KEY);
            return isSSO ? AccessTokenSource.FACEBOOK_APPLICATION_WEB
