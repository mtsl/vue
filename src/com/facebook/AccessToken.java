...
public final class AccessToken implements Serializable {
    ...
    public static AccessToken createFromExistingAccessToken(String accessToken,
        ...
        return new AccessToken(accessToken, expirationTime, permissions,
    ...
    static AccessToken createEmptyToken(List<String> permissions) {
        return new AccessToken("", ALREADY_EXPIRED_EXPIRATION_TIME,
                permissions, AccessTokenSource.NONE, DEFAULT_LAST_REFRESH_TIME);
    }
    ...
    static AccessToken createFromString(String token, List<String> permissions,
        ...
        return new AccessToken(token, DEFAULT_EXPIRATION_TIME, permissions,
                source, DEFAULT_LAST_REFRESH_TIME);
    }
    ...
    static AccessToken createFromRefresh(AccessToken current, Bundle bundle) {
        ...
        return createNew(current.getPermissions(), token, expires,
                current.source);
    }
    
    static AccessToken createFromTokenWithRefreshedPermissions(
            AccessToken token, List<String> permissions) {
        return new AccessToken(token.token, token.expires, permissions,
                token.source, token.lastRefresh);
    }
    
    private static AccessToken createNew(List<String> requestedPermissions,
            String accessToken, Date expires, AccessTokenSource source) {
        if (Utility.isNullOrEmpty(accessToken) || (expires == null)) {
            return createEmptyToken(requestedPermissions);
        } else {
            return new AccessToken(accessToken, expires, requestedPermissions,
                    source, new Date());
        }
    }
    ...
    static AccessToken createFromCache(Bundle bundle) {
        ...
        List<String> originalPermissions = bundle
                .getStringArrayList(TokenCachingStrategy.PERMISSIONS_KEY);
    ...
    private static AccessToken createFromBundle(
        ...
        return new AccessToken(token, expires, requestedPermissions, source,
                new Date());
    }
    ...
    private static class SerializationProxyV1 implements Serializable {
        ...
        private SerializationProxyV1(String token, Date expires,
                List<String> permissions, AccessTokenSource source,
                Date lastRefresh) {
            this.expires = expires;
        ...
        private Object readResolve() {
            return new AccessToken(token, expires, permissions, source,
                    lastRefresh);
        }
    }
    
    private Object writeReplace() {
        return new SerializationProxyV1(token, expires, permissions, source,
                lastRefresh);
    }
