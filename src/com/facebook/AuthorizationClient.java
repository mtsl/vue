...
class AuthorizationClient implements Serializable {
		...
		batch.addCallback(new RequestBatch.Callback() {
			...
			public void onBatchCompleted(RequestBatch batch) {
				try {
					...
					complete(Result.createErrorResult("Caught exception",
							ex.getMessage()));
				} finally {
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			if (!Utility.isNullOrEmpty(request.getPermissions())) {
				parameters.putString(ServerProtocol.DIALOG_PARAM_SCOPE,
						TextUtils.join(",", request.getPermissions()));
			}
		...
		void onWebDialogComplete(AuthorizationRequest request, Bundle values,
			...
			if (values != null) {
				...
				if (error instanceof FacebookOperationCanceledException) {
					outcome = Result
							.createCancelResult("User canceled log in.");
				} else {
					outcome = Result
							.createErrorResult(error.getMessage(), null);
				}
			}
	...
	class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
		...
		private Result handleResultOk(Intent data) {
			...
			if (errorType == null) {
				return Result.createTokenResult(AccessToken
				...
				return Result.createErrorResult(errorType,
	...
	class KatanaProxyAuthHandler extends KatanaAuthHandler {
		...
		private Result handleResultOk(Intent data) {
			...
			if (error == null) {
				...
				return Result.createErrorResult(error,
						extras.getString("error_description"));
			}
		}
	}
	...
	static class Result implements Serializable {
		...
		static Result createErrorResult(String errorType,
